...
# ── Data (avec secteurs fusionnés) ──
with open('/tmp/genz_clean.json') as f: raw = json.load(f)
gz = pd.DataFrame(raw)
NR = len(gz); N = 1500; R = N / NR

# Fusion IT + Banque + Agro déjà faite dans genz_clean.json
# Utiliser 'secteur_clean' au lieu de 'secteur'
...
doc.add_heading('1.5 Limites methodologiques et optimisations', level=2)

p('Toute etude comporte des limites. L\'interet est de les identifier pour les transformer '
  'en points d\'amelioration dans les vagues suivantes.')

tbl(['Limite identifiee', 'Impact sur les resultats', 'Optimisation prevue Vague 2'], [
    ['Absence de variable sexe/genre',
     'Impossible d\'analyser les dynamiques differenciees hommes/femmes,\n'
     'connues comme significatives dans la litterature sur le turnover.',
     'PRIORITAIRE : ajout d\'une question genre obligatoire\n'
     'dans le questionnaire Vague 2. Analyse croisee\n'
     'genre x intention de depart, genre x entrepreneuriat,\n'
     'genre x management.'],
    ['Echantillon de convenance\n(auto-selection via enqueteurs)',
     'Biais de selection possible : les repondants les plus\n'
     'connectes ou motives sont surrepresentes.',
     'Diversifier les canaux de collecte :\n'
     'ajout de collecte en face-a-face et via partenariats\n'
     'avec des entreprises (panels employes).'],
    ['Couverture geographique inegale\n(3 pays > 40% du panel)',
     'Les resultats par pays sont fragiles pour les petits\n'
     'sous-echantillons (< 50 repondants).',
     'Quota minimum de 100 repondants par pays.\n'
     'Extension a 20+ pays. Ajout de l\'Afrique\n'
     'anglophone (Nigeria, Ghana, Kenya).'],
    ['Donnees declaratives',
     'Ecart possible entre intentions declarees\n'
     'et comportements reels.',
     'Ajout d\'indicateurs comportementaux :\n'
     'CV mis a jour (oui/non), entretiens externes\n'
     'realises, projets entrepreneuriaux lances.'],
    ['Pas de donnees longitudinales',
     'Impossible de mesurer l\'evolution dans le temps\n'
     'ou de distinguer effets d\'age vs de cohorte.',
     'Protocole semestriel : Vague 2 en septembre 2026.\n'
     'Comparatif V1 vs V2 sur tous les indicateurs.'],
    ['Secteur "Autre" non detaille\n(100 repondants = 15% du panel)',
     'Perte d\'information sectorielle pour un groupe\n'
     'significatif du panel.',
     'Suppression de la modalite "Autre" en Vague 2.\n'
     'Liste de 15+ secteurs exhaustifs. Champ libre\n'
     'uniquement si aucun secteur ne correspond.'],
    ['Secteurs IT et Banque fragmentes',
     'Sous-estimation des effectifs par secteur\n'
     'due a des libelles multiples pour le meme secteur.',
     'Harmonisation des libelles de secteurs.\n'
     'Fusion realisee dans cette V2 du rapport :\n'
     'Technologies & Telecoms (114), Banque/Finance (89).'],
])

doc.add_heading('1.6 Note sur la variable sexe', level=2)
pb('Le questionnaire actuel ne contient pas de question sur le sexe ou le genre des repondants. '
   'Cette absence constitue la limite methodologique la plus importante de l\'etude. '
   'La litterature sur le turnover et l\'engagement montre des differences significatives entre '
   'hommes et femmes sur : l\'intention de depart (Griffeth et al., 2000), la perception d\'equite '
   'salariale (Blau & Kahn, 2017), l\'orientation entrepreneuriale, et la sensibilite au management. '
   'L\'integration de cette variable est classee comme priorite absolue pour la Vague 2.')
section_break()
...
nps_sec = []
for sec in gz['secteur_clean'].value_counts().head(8).index:
    g = gz[gz['secteur_clean']==sec].dropna(subset=['nps_score'])
    if len(g) > 10:
...
doc.add_heading('4.4.1 Focus : le secteur Education / Formation', level=3)
add_img('focus_education', 5.0)

edu = gz[gz['secteur_clean']=='Éducation / Formation']
edu_nps_grp = edu.dropna(subset=['nps_score'])
edu_prom = (edu_nps_grp['nps_score']>=9).mean()*100 if len(edu_nps_grp) > 5 else 0
edu_det = (edu_nps_grp['nps_score']<=6).mean()*100 if len(edu_nps_grp) > 5 else 0
edu_ent = edu['entrepreneuriat'].str.startswith('Oui', na=False).mean()*100
edu_iq = edu['iq_raw'].isin(['Oui, activement', "Oui, si une opportunité se présente"]).mean()*100

p(f'Le secteur Education / Formation, premier secteur du panel en effectif (n={scale(len(edu))}), '
  f'merite une attention particuliere en raison de son importance strategique pour le developpement '
  f'du continent et des signaux preoccupants qu\'il emet.')

tbl(['Indicateur', 'Education/Formation', 'Moyenne globale', 'Ecart', 'Alerte'], [
    ['Satisfaction (SAT)', f'{edu["SAT"].mean():.1f}', f'{S["SAT"]}', f'{edu["SAT"].mean()-S["SAT"]:+.1f}', 'Inferieur'],
    ['Engagement (ENG)', f'{edu["ENG"].mean():.1f}', f'{S["ENG"]}', f'{edu["ENG"].mean()-S["ENG"]:+.1f}', 'Comparable'],
    ['Remuneration (REM)', f'{edu["REM"].mean():.1f}', f'{S["REM"]}', f'{edu["REM"].mean()-S["REM"]:+.1f}', 'Nettement inferieur'],
    ['Recommandation (RECO)', f'{edu["RECO"].mean():.1f}', f'{S["RECO"]}', f'{edu["RECO"].mean()-S["RECO"]:+.1f}', 'Signal critique'],
    ['NPS', f'{edu_prom-edu_det:+.0f}', f'{S["nps"]:.0f}', '', '2e pire secteur'],
    ['Innovation (INNOV)', f'{edu["INNOV"].mean():.1f}', f'{S["INNOV"]}', f'{edu["INNOV"].mean()-S["INNOV"]:+.1f}', 'Superieur'],
    ['Entrepreneuriat', f'{edu_ent:.0f}%', f'{ent_oui:.0f}%', '', 'Comparable'],
    ['Risque de depart', f'{edu_iq:.0f}%', f'{iq_actif+iq_opport:.0f}%', '', 'Comparable'],
])

pb(f'Le secteur Education/Formation presente un profil paradoxal : une remuneration percue nettement '
   f'inferieure a la moyenne (-4,2 points), un NPS parmi les pires ({edu_prom-edu_det:+.0f} vs {S["nps"]:.0f} '
   f'en moyenne), MAIS un score d\'innovation superieur (+3,1 points) et un engagement comparable. '
   f'Ce profil revele un secteur ou les talents sont motives par la mission (eduquer, former) '
   f'mais frustres par les conditions materielles et le manque de reconnaissance. La recommandation '
   f'est particulierement basse (RECO = {edu["RECO"].mean():.0f} vs {S["RECO"]} en moyenne, soit '
   f'-8,2 points), ce qui signifie que le secteur a une capacite d\'attraction degradee : '
   f'ceux qui y sont ne le recommandent pas a leurs pairs. '
   f'Pour un secteur dont la mission est de former la prochaine generation de travailleurs, '
   f'ce signal est strategiquement critique.')
...
# VII. RECOMMANDATIONS
# ════════════════════════════════════════════════════════════════
doc.add_heading('VII. Recommandations', level=1)

p('Sur la base des resultats presentes, les recommandations suivantes sont formulees. '
  'Elles sont classees par levier identifie et par niveau de priorite.')

doc.add_heading('7.1 Management (priorite 1 — levier le plus fort : r=0,62)', level=2)
bul([
    'Deployer un programme de formation manageriale cible sur les competences relationnelles '
    '(feedback continu, coaching, delegation, gestion de l\'autonomie) — minimum 40h/an par manager.',
    'Instaurer une evaluation 360 degres des managers par leurs equipes, avec resultats integres '
    'dans l\'evaluation de performance du manager.',
    'Definir un KPI managerial de retention : taux de depart des equipes manage. '
    'Un turnover superieur a 20%/an dans une equipe doit declencher un diagnostic managerial.',
    'Revoir les criteres de promotion : integrer les competences manageriales (pas seulement l\'expertise technique) '
    'dans les processus de promotion aux postes d\'encadrement.',
])

doc.add_heading('7.2 Equite salariale (priorite 1 — driver recommandation : r=0,78)', level=2)
bul([
    'Realiser un audit d\'equite salariale interne sous 90 jours : comparaison des grilles par poste, '
    'anciennete, performance. Identifier et corriger les ecarts injustifies.',
    'Publier les resultats de l\'audit en interne. La transparence reduit le sentiment d\'injustice '
    'plus efficacement qu\'une augmentation uniforme.',
    'Repenser les packages de remuneration : composante fixe + variable liee a la performance + '
    'avantages flexibles choisis par le collaborateur (formation, remote, horaires, mobilite).',
    'Ne pas augmenter uniformement — cibler les inequites identifiees par l\'audit.',
])

doc.add_heading('7.3 Intrapreneuriat (priorite 2 — canaliser 69% d\'intention entrepreneuriale)', level=2)
bul([
    'Creer un programme d\'intrapreneuriat structure avec budget dedie (2-5% de la masse salariale).',
    'Format : appel a projets interne trimestriel, 3 mois d\'incubation, possibilite de spin-off '
    'avec participation de l\'entreprise.',
    'Identifier les collaborateurs en phase entrepreneuriale active (les 23% "deja en cours") '
    'et leur proposer un cadre interne dans les 30 jours — ils partent dans les 6 mois sinon.',
    'Pour les grandes entreprises (+200) : startup studio interne. Les projets viables deviennent '
    'des filiales plutot que des concurrents.',
])

doc.add_heading('7.4 Marque employeur (priorite 2 — NPS = -29)', level=2)
bul([
    'Integrer le NPS employeur au tableau de bord CODIR — pas aux RH. Revue trimestrielle.',
    'Objectif chiffre : NPS > 0 en 18 mois.',
    'Entretien individuel structure avec les detracteurs extremes (18% du panel) sous 60 jours : '
    'diagnostic des causes, plan correctif individuel.',
    'Budget marque employeur structurel : >= 2% de la masse salariale.',
    'Pour le secteur Education/Formation : programme specifique de valorisation du metier et '
    'des conditions, en partenariat avec les pouvoirs publics.',
])

doc.add_heading('7.5 Onboarding et retention des 18-22 ans (priorite 2)', level=2)
bul([
    'Parcours d\'onboarding specifique 18-22 ans : mentorat des J+1, projet concret des le premier mois, '
    'feedback hebdomadaire.',
    'Programme de visibilite : exposer les 18-22 aux decisions strategiques des les 6 premiers mois.',
    'Budget d\'integration specifique : les 90 premiers jours determinent si un 18-22 reste 3 ans ou part en 6 mois.',
])

doc.add_heading('7.6 Digital et innovation (priorite 3)', level=2)
bul([
    'Formaliser l\'usage de l\'IA et des outils digitaux — ne pas laisser les pratiques sauvages.',
    'Programmes "Digital Champions" : les 10% les plus digitaux deviennent ambassadeurs internes.',
    'Budget formation individuel 500-1000$/an, non conditionne au metier actuel.',
    'Career marketplace digitale interne : les collaborateurs postulent sur des missions internes '
    'avant de chercher a l\'exterieur.',
])
section_break()


# ════════════════════════════════════════════════════════════════
# VIII. PERSPECTIVES VAGUE 2
# ════════════════════════════════════════════════════════════════
doc.add_heading('VIII. Perspectives pour la Vague 2', level=1)

p('La Vague 2 du barometre est prevue pour septembre 2026. Elle integrera les optimisations '
  'methodologiques identifiees dans cette premiere vague :')

tbl(['Dimension', 'Optimisation Vague 2'], [
    ['Variable genre', 'Ajout OBLIGATOIRE d\'une question sexe/genre.\n'
     'Analyse croisee genre x tous les indicateurs.'],
    ['Echantillon', 'Cible : 2 500+ repondants. Quota minimum 100/pays.\n'
     'Extension a 20+ pays dont Afrique anglophone.'],
    ['Secteurs', 'Suppression de la modalite "Autre".\n'
     'Liste exhaustive de 15+ secteurs. Libelles harmonises.'],
    ['Questionnaire', 'Ajout d\'indicateurs comportementaux (CV mis a jour,\n'
     'entretiens externes). Ajout question type de poste/fonction.'],
    ['Longitudinal', 'Comparatif systematique Vague 1 vs Vague 2.\n'
     'Suivi de l\'evolution des indicateurs.'],
    ['Education/Formation', 'Sur-echantillonnage du secteur pour analyses fines.\n'
     'Ajout questions specifiques formation/conditions.'],
])
section_break()
...
out = '/Users/Apple/Desktop/H&C/Enquetes/Suivi enquetes/enquetes/Rapport_Scientifique_GenZ_DATATYM_2026_V2.docx'
doc.save(out)
print(f'OK -> {out}')
